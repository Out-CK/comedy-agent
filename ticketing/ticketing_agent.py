"""
ComedyTicketingAgent — queries Ticketmaster, SeatGeek, Eventbrite, and StubHub for
NYC comedy events, normalizes results into EventEntry objects, deduplicates,
...
class ComedyTicketingAgent:
    ...
    def run(self) -> None:
        ...
        logger.info(f"=== Comedy Ticketing Run START | entry_batch_id={entry_batch_id} ===")
        ...
        logger.info(f"Total raw comedy entries across all platforms: {len(all_entries)}")
        ...
        logger.info(
            f"=== Comedy Ticketing Run COMPLETE | entry_batch_id={entry_batch_id} | "
    ...
    def _fetch_ticketmaster(self) -> list[EventEntry]:
        with TicketmasterClient() as client:
            raw = client.fetch_nyc_comedy(days_ahead=DAYS_AHEAD)
