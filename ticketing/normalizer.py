"""
Convert raw API response dicts from each ticketing platform into comedy EventEntry objects.
"""
...
def _comedian_from_title(title: str) -> str:
    """Best-effort: extract comedian/show name from a title like 'Name at Venue'."""
...
def from_ticketmaster(event: dict) -> Optional[EventEntry]:
    try:
        ...
        if not artist_name:
            artist_name = _comedian_from_title(event_name) or event_name
        ...
        description = event.get("info") or event.get("description") or (
            f"{artist_name} performing live comedy at {venue_name}"
        )
...
def from_seatgeek(event: dict) -> Optional[EventEntry]:
    try:
        ...
        if not artist_name:
            artist_name = _comedian_from_title(event.get("short_title", "") or event.get("title", ""))
        ...
        url = event.get("url", "")
        title = f"{artist_name} at {venue_name}"
        description = f"{artist_name} performing live comedy at {venue_name}"
...
def from_eventbrite(event: dict) -> Optional[EventEntry]:
    try:
        ...
        description = (desc_info.get("text") or f"{artist_name} performing live comedy at {venue_name}")[:500]
...
def from_stubhub(event: dict) -> Optional[EventEntry]:
    try:
        ...
        artist_name = performers[0].get("name", "").strip() if performers else ""
        if not artist_name:
            artist_name = _comedian_from_title(name) or name
